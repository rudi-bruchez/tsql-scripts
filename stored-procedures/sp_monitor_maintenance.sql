...
AS BEGIN
	...
	--SELECT *
	--FROM MyDatabase.sys.index_resumable_operations iro
	...
	--WHERE instance_name = N'MyDatabase'
	WHERE instance_name = N'client'
	AND counter_name IN(N'Percent Log Used', N'Log File(s) Size (KB)')
	OPTION (RECOMPILE)
	*/

	EXEC sp_logspace 'MyDatabase'

	--USE [MyDatabase]
	--GO
	--DBCC SHRINKFILE (N'MyDatabasenew_log' , 10000)
	--GO
END